...
impl Attestation {
    ...
    fn verify_public_key_hash(
        ...
        Ok((
            pub_key_bytes,
            pub_key_hash.as_ref() == key_identifier.as_slice(),
        ))
    }

    fn _extract_client_pub_key_bytes(cert: &X509) -> Result<Vec<u8>, Box<dyn Error>> {
        ...
        let pub_key_bytes =
            ec_point.to_bytes(group, PointConversionForm::UNCOMPRESSED, &mut ctx)?;

        Ok(pub_key_bytes)
    }

    pub fn extract_client_pub_key_bytes(&self) -> Result<Vec<u8>, Box<dyn Error>> {
        let cred_cert = X509::from_der(&self.statement.certificates[0])?;
        let pub_key_bytes = Self::_extract_client_pub_key_bytes(&cred_cert)?;
